...
## Delete Events (Non-Interactive)

To delete from WhatsApp, use this pattern:
```bash
FILE=$(grep -rl "EVENT NAME" ~/.local/share/vdirsyncer/calendars/ | head -1) && rm "$FILE" && vdirsyncer sync && echo Done
```

## Edit Event Date (Non-Interactive)

To reschedule from WhatsApp, use sed on the .ics file:
```bash
FILE=$(grep -rl "EVENT NAME" ~/.local/share/vdirsyncer/calendars/ | head -1) && sed -i "s/OLD_DATE/NEW_DATE/g" "$FILE" && rm -f ~/.local/share/khal/khal.db && vdirsyncer sync && echo Done
```

## Edit Events (interactive - TTY only, NOT from WhatsApp)
