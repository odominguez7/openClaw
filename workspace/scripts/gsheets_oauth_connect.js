...
const CLIENT_PATH = process.env.GSHEETS_OAUTH_CLIENT || path.join(__dirname, '..', 'tmp', 'gsheets_oauth_client.json');
const TOKEN_PATH = process.env.GSHEETS_OAUTH_TOKEN || path.join(__dirname, '..', 'tmp', 'gsheets_oauth_token.json');
const PORT = parseInt(process.env.GSHEETS_OAUTH_PORT || '8765', 10);
...
async function main() {
  ...
  // Ensure redirect uri points to our local server
  ...
  // Try to open in a browser on the host (best-effort)
  try {
